...
public class SocketIO {
    ...
    public SocketIO (final String ip, final int port, final String[] allowedCorsOrigins) {
        PORT_START = new AtomicInteger(port);
        
        this.mPort = PORT_START.getAndIncrement();
        this.mServer = new Server(new InetSocketAddress(ip, this.mPort));
        this.eioOptions = EngineIoServerOptions.newFromDefault();
        this.eioOptions.setAllowedCorsOrigins(allowedCorsOrigins);
        
        this.mEngineIoServer = new EngineIoServer(this.eioOptions);
        ...
        */
        ...
        servletContextHandler.addServlet(new ServletHolder(new HttpServlet() {
            ...
            protected void service(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
                SocketIO.this.mEngineIoServer.handleRequest(new HttpServletRequestWrapper(request) {
                    @Override
        ...
        try {
            final WebSocketUpgradeFilter webSocketUpgradeFilter = WebSocketUpgradeFilter.configureContext(servletContextHandler);
